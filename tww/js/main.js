...
factionsData.TWW2 = {
    ...
    factions: ["Dark Elves", "The Blessed Dread", "Cult Of Pleasure", "High Elves", "Order Of Loremasters", "Nagarythe", "Lizardmen", "Last Defenders", "Itza", "Tlaqua", "Skaven", "Clan Pestilens", "Clan Rictus", "Karak Kadrin", "The Golden Order", "Middenland", "Mousillon", "The Barrow Legion", "Chevaliers De Lyonesse", "The Knights Of Caledor", "Bonerattlaz"]
};
...
factionsData.beastmen = {
    ...
    factions: ["Beastmen", "Warherd Of The Shadowgrave"]
};
...
const mapNames = {
    ...
    Ambush: ["Bloodpine Woods", "Broken Leg Gully", "Grey Guardians", "Grimminhagen", "Hel Fenn", "Ice Rock Gorge", "Lake Incatol", "Marbad's Tomb", "Obsidian Peaks", "Qu'ittax", "Shadow Wood", "Swamp Of Terror", "The Abyss", "The Black Deeps", "The Desert’s Bounty", "The Galleon's Graveyard", "The Great Forest", "The Heart Of The Dark", "The Wasteland", "Valley Of Thieves", "Witchwood Hollow"],
    ...
    Subterranean: ["Caverns Of The Great Bat", "Coldfire Cavern", "Gnaw Pit", "Gouger Gully", "Halls Of Karak Azul", "Lord Kroak's Temple", "Mortuary Of Tzulaqua", "Mushroom Cave", "Nexus Of The Oak", "Passages Of The Asrai", "Pits Of Zardok", "Shadow Forest", "Shadow Realm", "The Black Chasm", "The Black Deeps", "The Black Mountains", "The Death Pass Deeps", "The Great Forest", "The Heart Of The Dark", "The Thundering Falls", "Todtheim", "Volcanic Channels", "Warp-Shard Caverns", "Warpblade Tunnels"],
...
factionsTablesHTML = "";
var tmpheader, tmpdescription, tmpfactions, tmpfaction;
for (var i = 0; i < factionsHeaders.length; i++) {
    //Get the header
    tmpheader = factionsHeaders[i];

    //Get the description
    tmpdescription = factionsData[tmpheader].description;
    ...
    <tr>
    <th>
    <label class="checkbox-container">${tmpdescription}<input id="${tmpheader}" type="checkbox" onchange="checkChildren(this);"><span class="checkmark"></span></label>
    `

    //Get the factions of each header
    tmpFactions = factionsData[tmpheader].factions;
    for (var ii = 0; ii < tmpFactions.length; ii++) {
        //Get each faction
        tmpfaction = tmpFactions[ii];
        factionsTablesHTML += `<tr>
        <td>
        <label class="checkbox-container"><input name="${tmpheader}" type="checkbox" id="${tmpfaction}"><span class="checkmark"></span>${tmpfaction}</label>`
    }
...
function checkChildren(checkbox) {
    ...
    for (var j = 0; j < factionCheckboxes.length; j++) {
        factionCheckboxes[j].checked = status;
    }
}
...
function checkAll(isTrue) {
    ...
    let tmpCheckbox;
    for (var i = 0; i < factionsHeaders.length; i++) {
        //Get the checkbox with this header as id
        tmpCheckbox = document.getElementById(factionsHeaders[i]);
        //Check or uncheck this checkbox
        tmpCheckbox.checked = isTrue;
        //Update the children
        checkChildren(tmpCheckbox);
    }
}
...
function generateFactions() {
    //Get valid factions
    let validFactions = getValidFactions();
    ...
    let isForbidden = document.getElementById("factionForbidCheckbox").checked;
    ...
    for (var i = 0; i < choices.length; i++) {
        factionResultsHTML += `
        <br><button class="reloadButton" onclick="reloadFaction(this);" id="button_${i + 1}"><i class="fas fa-redo-alt fa-xs"></i></button>&emsp;<span id="span_${i + 1}" class="factionNameSpan">${i + 1}. ${choices[i]}</span><br>
        `;
    }
...
function reloadFaction(currentButton) {
    ...
    let textNumber, tmpText, tmpFactionName;
    for (var i = 0; i < allFactionNameSpans.length; i++) {
        //faction names are the names without the number and . and space
        tmpText = allFactionNameSpans[i].innerText;
        textNumber = i + 1;
        tmpFactionName = tmpText.replace(`${textNumber}. `, "");
        currentFactionNames.push(tmpFactionName);
    }
...
function getValidFactions() {
    ...
    //Go through each factin name:
    let tmpFactionName, tmpCheckbox;
    for (var i = 0; i < factionsNames.length; i++) {
        //Get the faction name
        tmpFactionName = factionsNames[i];

        //Getting the checkbox of this faction name
        tmpCheckbox = document.getElementById(tmpFactionName);

        //If that checkbox is checked, add the name
        if (tmpCheckbox.checked) {
            validFactions.push(tmpFactionName);
        }
    }
...
function getMaps() {
    ...
    let mapCheckbox, status, mapHeader, tmpMapNames, typeMapText;
    for (var i = 0; i < mapCheckboxes.length; i++) {
        //Get the checkbox
        mapCheckbox = mapCheckboxes[i];
        //get the status of this checkbox
        status = mapCheckbox.checked;
        //If this checkbox is checked, add its maps to the maps
        if (status) {
            //Get the map type header according to checkbox id
            mapHeader = mapCheckbox.id;

            //Get the maps under that type of header
            tmpMapNames = mapNames[mapHeader];

            //Go through them and add them to the maps
            for (var i = 0; i < tmpMapNames.length; i++) {
                //Make each map name the header type and the map
                typeMapText = `${mapHeader}: ${tmpMapNames[i]}`;
                maps.push(typeMapText);
            }
        }
    }
